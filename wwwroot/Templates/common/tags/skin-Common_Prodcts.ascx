...
<li onclick="location.href='/ProductDetails.aspx?ProductId=<%#Eval("ProductId") %>'">
					...
					<div class="y-shopinfo">
						...
						<div class="price">
	                        ¥<%# Eval("SalePrice", "{0:F2}") %><%#(Eval("SalePrice").ToString()==Eval("MaxShowPrice").ToString())?"":"起" %> <%#(Eval("MarketPrice")!=null&&Eval("MarketPrice","{0:F2}").ToString()!="0.00")?( "<del class=\"text-muted font-xs\">¥"+Eval("MarketPrice", "{0:F2}").ToString()+" </del>"):"" %>
	                    </div>
