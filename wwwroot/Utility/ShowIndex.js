...
$(function () {
    switch (GetPathName(window.location.pathname.toLowerCase())) {
        case "/default.aspx":
        ...
        case "/custom"://自定义页面
        case "/draftcustom"://草稿箱页面
            jQuery.getJSON("/api/Hi_Ajax_NavMenu.ashx?" + (new Date().getTime()), function (settingjson) {
                //底部导航
                menu(settingjson);
                //弹出圆圈菜单
...
function GetPathName(pathname) {
    ...
    var cuspathname = arr.length > 1 ? "/" + arr[1] : pathname;
    return cuspathname;
}
