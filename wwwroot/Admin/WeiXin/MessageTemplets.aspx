                ...
                <div class="mate-tabl">
                    ...
                    <div class="tab-content">
                        <div role="tabpanel" class="tab-pane active" id="home">
                            ...
                            <p class="y-textborderleft"> 微信消息模板ID设置，<a href="../WeiXin/weixinSettings.html" target="_blank">手动设置帮助？</a>　

                                <input type="button" onclick="onekeySet(this)" value="一键配置" class="btn btn-danger"/>
                                         ...
                                         <asp:Repeater runat="server" ID="rptList"   >
                                            <ItemTemplate>
                                                <tr>
                                                    ...
                                                    <td><%#Eval("WXOpenTM") %></td>
                                                    <td><asp:TextBox runat="server" shortid='<%#Eval("WXOpenTM") %>' ID="txtTemplateId" Text='<%#Eval("WeixinTemplateId") %>'  CssClass="inputw350" /></td>
                                                </tr>
                                            </ItemTemplate>
                                        </asp:Repeater>
...
<script type="text/javascript">
    ...
    ///一键配置模板
    function onekeySet(obj) {
        HiTipsShow("系统将自动尝试配置消息模板，如果配置成功后，请注意保存", "confirmII", function () {

            $(obj).prop("disabled",true)
            $.ajax({
                url: "/api/VshopProcess.ashx",
                type: "post",
                data: "action=onekeySet",
                datatype: "json",
                success: function (json) {
                    ...
                    if (json.errcode == 0)
                    {
                        ...
                        if (json.AppendData != null)
                        {
                            $.each(json.AppendData, function (i, item) {
                                $("input[shortid=" + item.shortId + "]").val(item.templateid);
                            });
                        }

                        HiTipsShow("自动配置成功，请检查，然后保存信息！", "success")
                    }
                    else
                    {
                        $(obj).prop("disabled", false);
                        HiTipsShow("配置失败：" + json.errmsg, "error")
                    }
