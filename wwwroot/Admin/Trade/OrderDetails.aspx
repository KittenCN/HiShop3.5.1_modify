...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <form runat="server">
                ...
                <div role="tabpanel" class="tab-pane active" id="home">
                    <div class="set-switch mt20">
                        ...
                        <ul class="clearfix buyerInfo">
                            <li style="width: 375px;">
                                ...
                                <div><span style="vertical-align:top;">付款方式：</span><span style="width: auto;text-align:left;"><asp:Literal ID="litPayType" runat="server"></asp:Literal></span></div>
                            </li>
                            <li>
                                <p class="mb10"><span>手机号：</span><asp:Literal runat="server" ID="litUserTel" /></p>
                                <p><span>收货时段：</span><asp:Literal ID="litShipToDate" runat="server"></asp:Literal></p>
                                <p class="mt10"><span>买家留言：</span><asp:Literal ID="litRemark" runat="server"></asp:Literal></p>
                            </li>
                    ...
                    <asp:Repeater ID="rptRefundList" runat="server" OnItemDataBound="rptRefundList_ItemDataBound">
                        <HeaderTemplate>
                            <div class="commissionInfo mb20">
                                <h3>退款信息</h3>
                                <a id="returnInfo" name="returnInfo"></a>
                                <div>
                        ...
                        <ItemTemplate>
                            <tr>
                                ...
                                <td style="padding: 15px; width: 30%; vertical-align: middle;">
                                    <p class="mb5" <%# string.IsNullOrEmpty(Server.HtmlEncode(Eval("Account").ToString()))?"style='display:none;'":"" %>>
                                        <span>退款帐号：</span>
                                        <%#Server.HtmlEncode(Eval("Account").ToString()) %>
                                    </p>
                                    <p class="mb5">
                                        <span>售后备注：</span><asp:Literal ID="litComments" runat="server"></asp:Literal>
                                        <%#Server.HtmlEncode(Eval("Comments").ToString()) %>
                                    </p>
                                    <p class="mb5">
                                        <span>退款金额：</span><span class="cssModify"><i style="color: red; font-weight: bold; font-style: normal;">￥<%#Eval("RefundMoney","{0:F2}") %></i>(含余额￥<%#Eval("BalancePayMoney","{0:F2}") %>)<a href="javascript:;" class="ml5 cssLinkModify" style="display: none;" id="linkModify" runat="server" title="修改退款金额">修改</a></span>
                                        <span class="cssCancelModify" style="display: none">
                                ...
                                <td style="border-left: 1px solid #ccc; padding: 15px; vertical-align: middle; width: 395px;">
                                    ...
                                    <p>
                                        <input type="button" class="btn btn-success btn-sm inputw100" value="同意退款" id="btnAgree" runat="server" returnsid='<%#Eval("ReturnsId") %>' skuid='<%#Eval("skuid") %>' orderitemid='<%#Eval("ID") %>' productid='<%#Eval("productid") %>' money='<%#Eval("RefundMoney","{0:F2}") %>' balancemoney='<%#Eval("BalancePayMoney","{0:F2}") %>' onclick="AgreeReturnFun($(this).attr('productid'), $(this).attr('skuid'), $(this).attr('money'), $(this).attr('returnsid'), $(this).attr('orderitemid'),$(this).attr('balancemoney'))" />
                ...
                <div role="tabpanel" class="tab-pane" id="profile">
                    <div class="set-switch">
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-10">
                                <span class="CompanyName">
                                    <asp:Literal ID="litCompanyName" runat="server" Text="-"></asp:Literal>
                                </span>
                                <input type="button" id="btnUpdateExpress" runat="server" class="btn btn-info btn-sm inputw100" value="修改物流" visible="false" onclick="UpdateExpress()" />
                            </div>
                        </div>
                        <p class="row">
                        ...
                        <div id="pLoginsticInfo" runat="server" class="row" visible="false">
                            ...
                            <span id="expressInfo" class="col-xs-10" style="color: #C60">正在加载中....<br />
                                <a href="http://www.kuaidi100.com" target="_blank" style="color: Red;">此物流信息由快递100提供</a></span>
                        </div>
                    </div>
                </div>
            </div>
                        ...
                        <div class="form-horizontal">
                            <div class="form-group">
                                ...
                                <div class="col-xs-9 pl0">
                                    <div><asp:TextBox ID="txtMoney" CssClass="form-control inputw200 inl" runat="server" onkeyup="TMsg()"></asp:TextBox> 元</div>
                                    <small id="balanceTips"></small>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <small class="col-xs-9 col-xs-offset-3 pl0">同意退款以后不可恢复，是否继续？</small>
                            </div>
                        </div>
        ...
        <!--编辑物流信息-->
        <div class="modal fade" id="OrderExpress">
            <div class="modal-dialog">
                <div class="modal-content form-horizontal">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                    ...
                    <div class="modal-body">
                        <table class="table">
                            ...
                            <tr>
                                <td><asp:Literal runat="server" ID="litName" /></td>
                                <td><asp:Literal runat="server" ID="litExOrderId" /></td>
                                <td><span class="getcompanyname" title=""></span></td>
                                <td><asp:TextBox runat="server" ID="txtShipOrderNumber" CssClass="inputw200"></asp:TextBox></td>
                                <td><Hi:OrderStatusLabel ID="OrderStatusLabelHtml" runat="server" /></td>
                            </tr>
                        </table>
                        <div class="modal-footer">
                            <input type="button" ID="btnSaveExpress" value="保存"   class="btn btn-success" />
                            <button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
                        </div>
                    </div>
                </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            //修改物流信息
            $("#btnSaveExpress").click(function () {
                var orderId = $("#ctl00_ContentPlaceHolder1_hdfOrderID").val();
                var obj = $("select[name='comp']");
                var compname = $(obj).find("option:selected").text();
                var companycode = $("select[name='comp']").val();
                if (companycode == "") {
                    ShowMsg("请选择快递公司", false);
                    return false;
                }
                var shipordernumber = $("input[name='ctl00$ContentPlaceHolder1$txtShipOrderNumber']").val();
                if (shipordernumber == "") {
                    ShowMsg("请输入快递单号", false);
                    return false;
                }
                ...
                $.ajax({
                    url: "sendordergoods.aspx",
                    type: "post",
                    data: "posttype=updateExpress&data=" + jdata,
                    datatype: "json",
                    success: function (json) {
                        if (json.type == "1") {
                            HiTipsShow(json.tips, "success", function () {
                                $(".close").trigger("click");
                                $(".CompanyName").html(compname);
                                $("#numberId").html(shipordernumber);
                            });
                        } else {
            ...
            if (orderId && document.getElementById("ctl00_ContentPlaceHolder1_pLoginsticInfo")) {
                ...
                if (expressData.message == "ok") {
                    if (hasNewKey == "1") {//付费接口通过订阅获取本地信息
                        ...
                        var message = expressData.message;
                        if (message == "ok") {
                            if (undefined == expressData.content.data) {
                                ...
                                html = expressData.content.reason;//快递鸟错误原因
                                $('#expressInfo').html(html);
                                return;
                            } else {
                                var data = expressData.content.data;
        ...
        function CreateSelectToObj(obj, jsondata) {
            var selHtml = "";
            selHtml = "<select name='comp' id='comp'><option value=''>请选择</option>";
        ...
        function UpdateExpress() {
            $("#OrderExpress").modal("toggle").children().css({
                width: '700px',
                height: '180px'
            });
        }
        ...
        /*同意退款*/
        function AgreeReturnFun(productid, skuid, money, returnsid, itemid,balance) {
            ...
            $("#ctl00_ContentPlaceHolder1_hdBalance").val(balance);
            var balancePayFreight = "<%= BalancePayFreightMoneyTotal%>";  //余额支付运费
            var balancePayTotal = "<%= BalancePayMoneyTotal%>";  //余额支付总额
            if (parseFloat(money) > parseFloat(balancePayTotal)) {
                $("#balanceTips").html("其中 ￥" + (parseFloat(balancePayTotal)).toFixed(2) + " 元退回用户的账户余额， 剩余 <span style='color:red;'>￥" + (parseFloat(money) - parseFloat(balancePayTotal)).toFixed(2) + "</span> 线下退回用户退款账号");
            } else if (parseFloat(money) <= parseFloat(balancePayTotal)) {
                $("#balanceTips").html("退款退回用户的账户余额");
            }
        ...
        /*退款金额输入，提示变化*/
        function TMsg() {
            var money = $("#ctl00_ContentPlaceHolder1_txtMoney").val();
            var balance = $("#ctl00_ContentPlaceHolder1_hdBalance").val();
            var balancePayFreight = "<%= BalancePayFreightMoneyTotal%>";  //余额支付运费
            var balancePayTotal = "<%= BalancePayMoneyTotal%>";  //余额支付总额
            if (parseFloat(money)) {
                if (parseFloat(money) > parseFloat(balancePayTotal)) {
