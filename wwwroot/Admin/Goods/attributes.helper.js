...
function prepareControls(isReset) {
    ...
    $.ajax({
        ...
        success: function (resultData) {
            ...
            console.log("BrandCategories:" + resultData.BrandCategories);
...
function prepareBrandCategories(brandCategories) {
    ...
    $.each(brandCategories, function (i, brand) {
        console.log("BrandId:"+brand.BrandId+"/BrandName:"+brand.BrandName);
...
function prepareSkus() {
    ...
    tableHeader.append($("<td align=\"center\">商品编码<\/td>"));
...
function checkSkuCode() {
    ...
    $.each($(".skuItem_SkuCode"), function() {
        if ($(this).val().length > 20) {
            ShowMsg("商品编码的长度不能超过20个字符！", false);
            $(this).focus();
            validated = false;
            return false;
        }
