<%@ Page Title="" Language="C#" MasterPageFile="~/Admin/SimplePage.Master" AutoEventWireup="true" CodeBehind="BatchPrintData.aspx.cs" Inherits="Hidistro.UI.Web.Admin.promotion.BatchPrintData" %>
                ...
                <table class="table table-bordered table-hover">
                    ...
                    <tbody>
                        <asp:Repeater ID="rptItemList" runat="server">
                            <ItemTemplate>
                                <tr>
                                    <td title="<%#Eval("ReggionPath").ToString()+", "+Eval("Address").ToString().Trim() %>"><%#Eval("Receiver") %></td>
                                    <td title='orderid' logid="<%#Eval("LogId") %>" pid="<%#Eval("Pid") %>"><%# (int)Eval("Ptype")==1? Eval("PrizeNums").ToString().Remove(Eval("PrizeNums").ToString().Length-1):Eval("WinTime","{0:yyyy-MM-dd-}")+Eval("LogId").ToString() %></td>
                                    <td>
                                        <input type="text" name="shipordernumber" style="width: 160px;" value="<%#Eval("CourierNumber") %>" tid="<%#Eval("rownumber") %>" title="快递单号" maxlength="20" onkeyup="value=value.replace(/[\W]/g,'')" /></td>
                                </tr>
                            </ItemTemplate>
                        </asp:Repeater>
                    </tbody>
            ...
            $("#btnPrintAll").click(function () {
                ...
                if (ispass) {
                    $(".table-bordered tbody tr").each(function () {
                        ...
                        var orderid = $.trim($(this).find("td[title='orderid']").attr("logid"));
                        if (orderid == "0") {
                            orderid = "";
                        }
                        var pid = $.trim($(this).find("td[title='orderid']").attr("pid"));
                        if (pid == "0") {
                            pid = "";
                        }
                        ...
                        if (ispass) {
                            if (jdata == "") {
                                jdata = "{\"orderid\":\"" + String2Json(orderid) + "\",\"shipordernumber\":\"" + String2Json(shipordernumber) + "\",\"pid\":\""+String2Json(pid)+"\"}";
                            } else {
                                jdata += ",{\"orderid\":\"" + String2Json(orderid) + "\",\"shipordernumber\":\"" + String2Json(shipordernumber) + "\",\"pid\":\"" + String2Json(pid) + "\"}";
                            }
                        }
