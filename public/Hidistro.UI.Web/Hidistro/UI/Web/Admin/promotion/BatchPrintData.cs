namespace Hidistro.UI.Web.Admin.promotion
{
   using  global:: ControlPanel.Promotions;
    ...
    public class BatchPrintData : AdminPage
    {
        ...
        {
            DataSet prizeListByLogIDList = GameHelper.GetPrizeListByLogIDList(orderlist, pidlist);
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            pidlist = Globals.RequestQueryStr("PIds").Trim(new char[] { ',' });
            switch (str)
            {
                case "printall":
                {
                        ...
                        if (((shipperId > 0) && !string.IsNullOrEmpty(str5)) && !string.IsNullOrEmpty(company))
                        {
                            ...
                            if (array2.Count > 0)
                            {
                                foreach (JObject obj3 in array2)
                                {
                                    ...
                                    str7 = str7 + "," + obj3["pid"].ToString();
                                    GameHelper.SetPrintOrderExpress(obj3["orderid"].ToString(), obj3["pid"].ToString(), company, expressCompanyAbb, obj3["shipordernumber"].ToString());
                                }
                                this.printdata(shipperId, str5, str6.Trim(new char[] { ',' }), str7.Trim(new char[] { ',' }), iiscombine);
                            }
                        }
                    }
                    return;
                ...
                default:
                {
                    ...
                    if (string.IsNullOrEmpty(orderlist) && string.IsNullOrEmpty(pidlist))
                    {
                        return;
                    }
                    int num3 = !string.IsNullOrEmpty(orderlist) ? orderlist.Split(new char[] { ',' }).Length : 0;
                    int num4 = !string.IsNullOrEmpty(pidlist) ? pidlist.Split(new char[] { ',' }).Length : 0;
                    this.litNumber.Text = (num3 + num4).ToString();
            ...
            this.LoadShipper();
            this.LoadTemplates();
            this.LoadData(orderlist, pidlist);
        }
        ...
        private void printdata(int shipperId, string lgFile, string neworderlist, string newpidlist, int iiscombine)
        {
            ...
            else
            {
                ...
                if (File.Exists(path))
                {
                    ...
                    DataTable table = this.GetPrintData(neworderlist, newpidlist).Tables[0];
                    ...
                    for (int i = 0; i < table.Rows.Count; i++)
                    {
                        ...
                        if (iiscombine == 1)
                        {
                            ...
                            {
                                if ((((num2 + 1) >= table.Rows.Count) || !(table.Rows[num2]["CourierNumber"].ToString() == table.Rows[num2 + 1]["CourierNumber"].ToString())) || !(table.Rows[num2]["Receiver"].ToString() == table.Rows[num2 + 1]["Receiver"].ToString()))
                                {
                                    break;
                                }
                                this.pringrows--;
                        ...
                        string[] strArray = row["ReggionPath"].ToString().Split(new char[] { ',' });
                        string[] strArray2 = RegionHelper.GetFullRegion(Globals.ToNum(strArray[strArray.Length - 1]), "-").Split(new char[] { '-' });
                        foreach (XmlNode node2 in node.SelectNodes("item"))
                        {
                            ...
                            if (shipper != null)
                            {
                                strArray3 = RegionHelper.GetFullRegion(shipper.RegionId, "-").Split(new char[] { '-' });
                            }
                            ...
                            switch (str12)
                            {
                                case "收货人-姓名":
                                {
                                    this.ShipTo = this.ShipTo + "'" + this.ReplaceString(row["Receiver"].ToString()) + "',";
                                    if (string.IsNullOrEmpty(row["Receiver"].ToString().Trim()))
                                    {
                                        break;
                                    }
                                ...
                                case "收货人-电话":
                                {
                                    this.ShipTelPhone = this.ShipTelPhone + "'" + row["Tel"].ToString() + "',";
                                    if (string.IsNullOrEmpty(row["Tel"].ToString().Trim()))
                                    {
                                        goto Label_0652;
                                    }
                                ...
                                case "收货人-手机":
                                {
                                    this.ShipCellPhone = this.ShipCellPhone + "'" + row["Tel"].ToString() + "',";
                                    if (string.IsNullOrEmpty(row["Tel"].ToString().Trim()))
                                    {
                                        goto Label_06D8;
                                    }
                                ...
                                case "送货-上门时间":
                                {
                                    this.ShipToDate = this.ShipToDate + "'" + row["DeliveryTime"].ToString() + "',";
                                    if (string.IsNullOrEmpty(row["DeliveryTime"].ToString().Trim()))
                                    {
                                        goto Label_09D3;
                                    }
                                ...
                                case "订单-详情":
                                    str13 = string.Empty;
                                    str13 = string.Concat(new object[] { str13, "奖品名称：", row["ProductName"], " " }).Replace(";", "");
                                    if (num2 <= i)
                                    {
                                        goto Label_0CC4;
                                    }
                        ...
                        Label_09D3:
                            ...
                            continue;
                        Label_0A07:
                            builder.Append(";" + ((Globals.ToNum(table.Rows[num3]["Ptype"]) == 1) ? table.Rows[num3]["PrizeNums"].ToString().Remove(table.Rows[num3]["PrizeNums"].ToString().Length - 1) : (Convert.ToDateTime(table.Rows[num3]["WinTime"]).ToString("0:yyyy-MM-dd-") + table.Rows[num3]["LogId"].ToString())));
                            num3++;
                        Label_0AD4:
                            if (num3 < (num2 + 1))
                            {
                            ...
                            this.OrderId = this.OrderId + "'中奖编号：" + builder.ToString().Trim(new char[] { ';' }) + "',";
                            goto Label_0BAF;
                        Label_0B1B:
                            this.OrderId = this.OrderId + "'中奖编号：" + ((Globals.ToNum(row["Ptype"]) == 1) ? row["PrizeNums"].ToString().Remove(row["PrizeNums"].ToString().Length - 1) : (Convert.ToDateTime(row["WinTime"]).ToString("yyyy-MM-dd-") + row["LogId"].ToString())) + "',";
                        Label_0BAF:
                            if (!string.IsNullOrEmpty(row["LogId"].ToString().Trim()))
                            {
                        ...
                        Label_11FB:
                            ...
                            {
                                string[] strArray4 = str4.Split(new char[] { '_' });
                                if ((strArray4.Length == 3) && (strArray4[1] == "自定义内容"))
                                {
                                    this.SelfDefinedContentLength++;
                                    this.SelfDefinedContent = this.SelfDefinedContent + "'" + this.ReplaceString(strArray4[0]) + "',";
